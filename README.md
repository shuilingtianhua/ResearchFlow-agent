...
当前已经完成 M1 可运行执行闭环、M2 持久化与运行治理和 M3.1 产物闭环：提交研究目标后，系统会生成固定任务图，把状态、事件和 Task ArtifactRef 原子写入 SQLite，再由后台 Worker 在 `RunBudget.max_concurrency` 限制内并行执行无依赖冲突的 Fake Capability。文件产物使用可配置的 SHA-256 内容寻址目录保存；服务重启时会把中断执行恢复为可安全 Resume 的暂停状态，客户端可通过 SSE 按事件序号实时续读。
...
- [M3.1 内容寻址 Artifact Store 与 Runtime 产物闭环实施记录](docs/development/2026-08-16_m3_1_artifact_store_runtime.md)
...
$env:RESEARCHFLOW_ARTIFACT_ROOT = "D:/data/researchflow-artifacts"
python -m researchflow
```
